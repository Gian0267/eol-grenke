...
**Status: LIVE at https://eol.smartcomgroup.it (Hostinger), currently in TEST PHASE** — real cloud infra (Supabase DB+Storage, Resend, Aruba PEC) but with test-only helpers active (see "Test phase" section). Company renamed to **Integra Solutions Srl** (was Smartcom Solutions Srl; same P.IVA 12711040019 and sede Via Tunisia 5, Collegno TO). **Domain migration to `eol.noleggiosumisura.it` in progress**: DNS A record already points to Hostinger, but the domain is not yet added in hPanel — so `FRONTEND_URL` and every logo URL in the email templates deliberately still use `eol.smartcomgroup.it`. Switch them, plus `aggiorna-branding-integra.ts --dominio` for the DB template copies, only after the hPanel cutover. Linked from the smartcomgroup.it area riservata ("Fine Noleggio EOL" card, separate login — no SSO by explicit choice).
