...
NSM EOL Grenke — end-of-lease management platform for Grenke FLEX contracts, built for Integra Solutions / Noleggio Su Misura. Manages the full lifecycle: Excel import → automatic reconciliation → client communication → client decision → payment → closure.

**Status: LIVE at https://eol.noleggiosumisura.it (Hostinger), currently in TEST PHASE** — real cloud infra (Supabase DB+Storage, Resend, Aruba PEC) but with test-only helpers active (see "Test phase" section). Company renamed to **Integra Solutions Srl** (was Smartcom Solutions Srl; same P.IVA 12711040019 and sede Via Tunisia 5, Collegno TO). **Domain cutover done on 15/09/2026**: the app moved from `eol.smartcomgroup.it` to `eol.noleggiosumisura.it` (hPanel "Cambia dominio" replaces the hostname — Node web apps accept only one), `FRONTEND_URL`, the Stripe live webhook and every logo URL were switched over. The old hostname now returns 403; the "Fine Noleggio EOL" card in the smartcomgroup.it area riservata still points at it and needs updating (other project). Linked from the smartcomgroup.it area riservata ("Fine Noleggio EOL" card, separate login — no SSO by explicit choice).
...
- **Live URL**: https://eol.noleggiosumisura.it — Hostinger shared "Web app Node.js" (hPanel), **auto-deploys on every push to `main`** of GitHub repo `Gian0267/eol-grenke`.
...
- **Services**: `reconciliation.service.ts` (Excel parsing + DB matching), `pricing.service.ts` (canone calculations + valore Sconto Bronze), `email.service.ts` (comunicazione iniziale via SMTP), `storage.service.ts` (document storage abstraction), `codice-sconto.service.ts` (codici Sconto Copertura Bronze: generazione idempotente, marca utilizzato, annulla, scadenza)
- **Premio Fedeltà: Sconto Copertura Bronze** (replaced the gift card, June 2026): renewal incentive = discount on the BRONZE accidental-damage coverage of the new FLEX contract. On renewal confirmation (both `/decisione/rinnovo/conferma` and the compound `/decisione/rinnovo-completo/conferma` used by the wizard, plus backoffice `decisione-manuale` RINNOVO) the app generates an opaque single-use discount code `NSM-XXXX-XXXX` (crypto.randomBytes, charset without 0/O/1/I/L, table `Codice_Sconto`, states GENERATO/UTILIZZATO/SCADUTO/ANNULLATO, value from `contratto.valore_gift_card`, validity from `pricing.gift_card_validita_mesi`). Code shown in client flow step 4, conferma_rinnovo + notifica_agente emails (conditional `{{#if codice_sconto}}` blocks), conferma rinnovo PDF, PraticaDettaglio card, and managed in the backoffice "Codici Sconto" page (`codici-sconto.routes.ts`, BACKOFFICE_INTERNO/ADMIN, CSV export). Daily scheduler expires overdue codes. All state transitions audited (CODICE_SCONTO_*). The rate-reduction calculation (value vs finance coefficient, floor at zero) lives on the external rental platform — OUT OF SCOPE here. **IMPORTANT — internal keys were deliberately NOT renamed** for backward compat: `valore_gift_card` (DB column/API field/Handlebars var), `pricing.gift_card_tagli`, `pricing.gift_card_validita_mesi`, `flags.abilita_gift_card`, `[VALORE_GIFT_CARD]` script placeholder. Only labels/visible texts changed; `{{valore_sconto_bronze}}` is an alias of `{{valore_gift_card}}`. To update texts on an existing install run `npx tsx backend/scripts/aggiorna-testi-sconto-bronze.ts` (smart upsert: preserves admin-customized values).
- **Document storage** (`storage.service.ts`): PDFs (verbale restituzione, conferma rinnovo, ricevuta pagamento, audit export) are generated in-memory as Buffers and saved via `saveDocument(buffer, filename)`. Backend auto-selected: **Supabase Storage** when `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` set (private bucket `SUPABASE_BUCKET`, default `documenti`; DB ref stored as `supabase:<key>`), else **local disk** fallback (`backend/storage/pdfs/`, ref = absolute path). `loadDocument(ref)` auto-detects ref type, so legacy local paths still resolve. PDF gen functions return `{ pdfPath, hash, buffer }` — callers use the returned `buffer` directly. The mock FEA signature provider takes a Buffer (not a path). PDF content notes: verbale restituzione has the "divisione rental di Integra Solutions" subtitle and a bold non-return charge clause = pricing_grenke × 1.2; the yellow "modalità MOCK" box was removed from verbale and conferma rinnovo (the FEA signature is STILL a mock under the hood — integrating a certified eIDAS FEA provider remains an open pre-production decision).
- **Providers**: `providers/notification/email.provider.ts` — email via `createEmailProvider()` factory: **Resend** (`ResendEmailProvider`) when `RESEND_API_KEY` set, else **SMTP/nodemailer** (`SmtpEmailProvider`, Mailpit in dev). Both implement the same `EmailProvider` interface (`send`, `sendWithAttachment`). Sender = `RESEND_FROM` (`info@noleggiosumisura.it`, domain `noleggiosumisura.it` verified on Resend). Instantiate via `createEmailProvider()`, never `new SmtpEmailProvider()` directly.
...
- **Email templates**: `templates/email/` — Handlebars HTML templates with inline CSS. **The live template is the DB copy** (Impostazione rows, read via `configService.getHtml('email.<nome>')`); the file is only a fallback and the seed source. Editing a template file does NOT change production — update the Impostazione row too (or via the Impostazioni admin UI). The PEC channel of the comunicazione iniziale uses a dedicated institutional template (`comunicazione_iniziale_pec.html` / key `email.comunicazione_iniziale_pec`): sober serif style, no emoji/colored boxes. Branding on all templates: header "Integra Solutions Srl" with small "Noleggio Su Misura — Divisione Rental di Integra Solutions Srl". No colored-circle emojis in option headings (removed from comunicazione iniziale + 4 solleciti; colored left borders remain).
- **OTP** (`otp.service.ts`): 6-digit code, 10-min validity, stored in OtpCode and **actually emailed** via the email provider. **EMAIL is the only channel**: the SMS option was removed from the 3 client flows and the zod schemas accept only `EMAIL` (no SMS provider was ever integrated). Test backdoor `123456` works only when `NODE_ENV !== 'production'` + feature flag; the matching amber banner in the 3 client flows renders only in dev builds (`import.meta.env.DEV`).
...
- `loyalty_program.json` — Premio Fedeltà / Sconto Bronze standard cuts (internal key still `gift_card`)
...
- **`TEST_MAIL_REDIRECT` env var** (NOT set in hPanel as of 04/09/2026 — verified from the runtime log, which does not print the "MODALITÀ TEST" line at startup: **LIVE-ambiente pratiche send for real to clients, and have done so for months**. Still set in local .env = g.ciardo@gmail.com): `TestRedirectEmailProvider` reroutes EVERY outgoing email to that address with subject prefix `(mail cliente)` / `(pec cliente)` and an amber banner showing the original recipient. With it set, the PEC channel does NOT touch Aruba (zero real PEC consumed). Remove the var → real sending resumes. **Independently of this var, pratiche with `ambiente = 'TEST'` are ALWAYS redirected** by `emailProviderPerAmbiente` ([email.provider.ts](backend/src/providers/notification/email.provider.ts)), falling back to a hardcoded `g.ciardo@gmail.com`: that, not the env var, is what keeps the 15 test pratiche from writing to anyone.
...
- **Pricing**: monte_canoni = canone × mesi; **pricing_grenke** (what Grenke charges Smartcom) comes from the Grenke Excel file (column "Prezzo Riacquisto Grenke", REQUIRED — missing/invalid → row ERRORE at import); **pricing_riacquisto** (price charged to the client) = canone_mensile × (numero_mesi / 12) × **mensilità per anno**, configurable per origin in Impostazioni → Pricing: `pricing.mensilita_per_anno` (default 1, everything except Italiaonline) and `pricing.mensilita_per_anno_iol` (default 1.5), the origin matched tolerantly against `iol.diciture_origine` via `lib/origine.ts`. Changing them affects **future imports only** — existing pratiche are never recalculated. For a one-off price on a single client, BACKOFFICE_INTERNO/ADMIN can override `pricing_riacquisto` from the pratica detail ("Prezzo di riacquisto"), but **only while stato = LISTA_RICEVUTA**, i.e. before the comunicazione iniziale goes out; afterwards the only lever is the partial buyback. `pricing.riacquisto_percentuale` was deleted (dead config); margine_lordo = riacquisto − grenke (real difference, no fixed %); Sconto Bronze (ex gift card, field `valore_gift_card`) = floor of margine to nearest standard cut. **IVA: ordinary 22% on the full taxable amount** (`iva_percentuale` in pricing_rules.json). The previous *IVA a margine* (VAT on the margin only, used-goods regime) was dropped on 04/09/2026 by business decision, before any payment had been registered — so no receipt needed correcting. It also removed an inconsistency: the payment-invitation email already charged full VAT while the client area and the receipt used margin VAT. The old 5%/8% percentages in pricing_rules.json / Impostazioni are obsolete (only legacy fallbacks).
...
- **Deferred buyback payment**: the payment link is sent at **T-21** (21 days before expiry; was T-7 — config key `timeline.pagamento_riacquisto`, default 21 in code and `config/timeline.json`). Within T-21 the payment is immediate.
...
- **PEC env vars**: `PEC_SMTP_HOST` (default `smtps.pec.aruba.it`), `PEC_SMTP_PORT` (465), `PEC_USER`, `PEC_PASSWORD`, `PEC_FROM`. Current mailbox: `integra@pec.integrasystems.it` (custom PEC domain — confirm the SMTP host with the provider, `smtps.pec.aruba.it` only applies if it is Aruba-hosted). **Aruba PEC 2FA gotcha**: if the mailbox has 2-step verification, normal SMTP login fails with `535 Authentication failed` — you must generate an app-specific password ("password per email clients" in the Aruba panel) and use it as `PEC_PASSWORD`. Repeated failed logins can temporarily lock SMTP access.
