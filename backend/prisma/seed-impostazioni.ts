...
export async function seedImpostazioni(prisma: PrismaClient) {
  ...
  const impostazioni: ImpostazioneSeed[] = [
    ...
    { chiave: 'timeline.pagamento_riacquisto', valore: String(pre.pagamento_riacquisto ?? 23), tipo: 'NUMERO', categoria: 'TIMELINE', label: 'Richiesta pagamento riacquisto', descrizione: 'Giorni prima della scadenza per l\'invio della richiesta di pagamento del riacquisto (anticipata rispetto alla verifica a T-21)' },
