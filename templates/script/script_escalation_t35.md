# Script di chiamata — Terzo e ultimo tentativo (T-35)

## Apertura

"Buongiorno, sono [NOME AGENTE] di Noleggio Su Misura. La contatto con urgenza in merito al contratto n. [CONTRATTO_NSM]. Parlo con il referente?"

## Comunicazione di urgenza

"Questa è la mia ultima chiamata relativa al Suo contratto di locazione operativa per [BENI], in scadenza il [DATA_SCADENZA]. Il termine per comunicare la Sua decisione è il [DEADLINE], ovvero tra pochissimi giorni."

## Conseguenze esplicite

"Desidero informarLa chiaramente su cosa accade se non riceviamo la Sua scelta entro il termine:

- Il contratto Grenke proseguirà **automaticamente in proroga per 6 mesi** con canoni invariati
- Al termine della proroga, **non sarà più possibile** accedere alle opzioni di riacquisto, rinnovo con premio fedeltà o restituzione agevolata attraverso Noleggio Su Misura
- Grenke proseguirà nella gestione del contratto direttamente con la Sua azienda

Questo significa che le condizioni vantaggiose che Le abbiamo proposto non saranno più disponibili."

## Proposta di azione immediata

"Possiamo risolvere la questione adesso in pochi minuti. Le riassumo le opzioni:

1. **Rinnovo** — nuovo contratto su dispositivi aggiornati + gift card da [VALORE_GIFT_CARD]€
2. **Riacquisto** — i beni diventano Suoi a [PRICING_RIACQUISTO]€ + IVA
3. **Contatto** — fissiamo subito un appuntamento con un consulente
4. **Restituzione** — Le indichiamo come procedere alla riconsegna

Ha già una preferenza? Posso registrare la Sua scelta direttamente adesso, senza bisogno di accedere alla piattaforma."

## Gestione obiezioni

- **"Non mi interessa, lasciate in proroga"** → "Comprendo la Sua posizione. Tenga presente che la proroga comporta il pagamento di ulteriori 6 mesi di canone a [CANONE_MENSILE]€/mese, per un totale di [CANONE_6_MESI]€. Le opzioni che Le proponiamo potrebbero risultare più convenienti. Desidera valutare?"
- **"Registrate pure la restituzione"** → "Certamente, registro la Sua scelta di restituzione. Riceverà le istruzioni operative via email. Posso procedere?"
- **Nessuna risposta / segreteria** → Lasciare messaggio: "Buongiorno, sono [NOME] di Noleggio Su Misura. La contatto urgentemente per il contratto n. [CONTRATTO_NSM] in scadenza. Il termine per la scelta è il [DEADLINE]. La prego di contattarci al 011 4557949 o di accedere all'area riservata tramite il link nella email. Grazie."

## Chiusura

"La ringrazio per il Suo tempo. Questa è l'ultima possibilità per comunicarci la Sua preferenza prima del termine del [DEADLINE]. Resto a disposizione al numero 011 4557949. Buona giornata."
